...
shellkitpm_version=1.0.3  # Initial bootstrap version.  You can always do `shpm install shellkit-pm` to update it

canonpath() {
    ( cd -L -- "$(command dirname -- "${1}")" || exit; echo "$(command pwd -P)/$(command basename -- "${1}")" )
}
...
main() {
    ...
    local tmpDir; tmpDir=$(mktemp -d "${TMPDIR:-/tmp}/shk-bootstrap.XXXXXX")
    (
